(() => {
  ...
  function shellMarkup(ctx, message){
    return `<div class="game-day-pulse-head">
      <div>
        <div class="game-day-pulse-eyebrow">Cross-league player pulse</div>
        <div class="game-day-pulse-title">Your Week ${ctx.week} impact leaders</div>
        <div class="game-day-pulse-sub">Top starters across your Sleeper leagues, adjusted for each league’s scoring and matchup leverage.</div>
      </div>
      <div class="game-day-pulse-badge">Week ${ctx.week}</div>
    </div>
    <div class="game-day-pulse-grid">
      <div class="game-day-pulse-column">
        <div class="game-day-pulse-column-head"><span>Most Important Players</span><small>Top 5</small></div>
      ...
      <div class="game-day-pulse-column villains">
        <div class="game-day-pulse-column-head"><span>Villains of the Week</span><small>Top 5 threats</small></div>
  ...
  function addExposure(map,id,side,entry,record){
    if (!map.has(id)){
      map.set(id,{id,record,owned:[],against:[],ownedRaw:0,againstRaw:0,ownedWeighted:0,againstWeighted:0});
    }
  ...
  async function loadPulse(ctx){
    ...
    const promise = (async()=>{
      ...
      return {
        important:all.filter(item=>item.owned.length).sort((a,b)=>b.netFor-a.netFor || b.ownedWeighted-a.ownedWeighted || b.ownedRaw-a.ownedRaw).slice(0,5),
        villains:all.filter(item=>item.against.length && item.netAgainst>.05).sort((a,b)=>b.netAgainst-a.netAgainst || b.againstWeighted-a.againstWeighted || b.againstRaw-a.againstRaw).slice(0,5)
      };
    })();
  ...
  function card(item,type,index){
    ...
    const score = villain ? item.netAgainst : item.netFor;
    const primary = villain ? item.againstRaw : item.ownedRaw;
    const offset = villain ? item.ownedRaw : item.againstRaw;
    ...
    return `<article class="game-day-pulse-card ${villain?"villain":"important"}">
      ...
      <div class="game-day-pulse-score">
        <strong>${score.toFixed(1)}</strong><span>${villain?"Threat":"Importance"}</span>
        <small>${villain?"Against":"For"} ${primary.toFixed(1)}${offset>0?` · offset ${offset.toFixed(1)}`:""}</small>
      </div>
    </article>`;
  }
  ...
  async function render(force=false){
    ...
    const host = mount();
    if (!ctx.username){
      host.innerHTML = shellMarkup(ctx,"Enter your Sleeper username to build the player pulse.");
      return;
    }
    if (!force && ctx.key === lastContextKey && host.dataset.loadedKey === ctx.key) return;
    lastContextKey = ctx.key;
    host.innerHTML = shellMarkup(ctx);
    try {
      const data = await loadPulse(ctx);
      if (context().key !== ctx.key) return;
      const important = data.important.length ? data.important.map((item,i)=>card(item,"important",i)).join("") : `<div class="game-day-pulse-empty">No qualifying starters found for this week.</div>`;
      const villains = data.villains.length ? data.villains.map((item,i)=>card(item,"villain",i)).join("") : `<div class="game-day-pulse-empty">No net opponent threats found for this week.</div>`;
      host.innerHTML = `<div class="game-day-pulse-head"><div><div class="game-day-pulse-eyebrow">Cross-league player pulse</div><div class="game-day-pulse-title">Your Week ${ctx.week} impact leaders</div><div class="game-day-pulse-sub">Top starters across your Sleeper leagues, adjusted for each league’s scoring and matchup leverage.</div></div><div class="game-day-pulse-badge">Week ${ctx.week}</div></div><div class="game-day-pulse-grid"><div class="game-day-pulse-column"><div class="game-day-pulse-column-head"><span>Most Important Players</span><small>Top 5</small></div><div class="game-day-pulse-list">${important}</div></div><div class="game-day-pulse-column villains"><div class="game-day-pulse-column-head"><span>Villains of the Week</span><small>Top 5 threats</small></div><div class="game-day-pulse-list">${villains}</div></div></div>`;
      host.dataset.loadedKey = ctx.key;
    } catch (error){
      host.innerHTML = shellMarkup(ctx,error?.message || "Player pulse could not be loaded.");
    }
  }
  ...
  function init(){
    mount();
    render(true);
    document.addEventListener("input", event => { if (event.target?.matches?.("#username,#sleeperUsername,input[name='username']")) schedule(true); });
    document.addEventListener("change", event => { if (event.target?.matches?.("#username,#sleeperUsername,#week,#weekSelect,#week-selector,#season,#seasonSelect,input[name='username'],select[name='season'],select[name='week']")) schedule(true); });
    window.addEventListener("fantasy-settings-changed",()=>schedule(true));
    setInterval(()=>{
      mount();
      const key = context().key;
      if (key !== lastContextKey) schedule(true);
    },900);
  }
